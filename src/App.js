...
function App() {
  ...
  const [todos, setTodos] = useState([{
    ...
    id: 2, content: 'hey!yo', isDone: true
  }])
  //controllered
  ...
  return (
    <div className="App">
      ...
      <button style={{
        marginLeft: '5px'
      }} onClick={handleButtonClick}>Add todo</button>
      {
        todos.map(todo => <TodoItem key={todo.id} todo={todo} handleTodoIsDone={handleTodoIsDone} handleDeleteItem={handleDeleteItem} />)
      }
    </div>
  );
}
